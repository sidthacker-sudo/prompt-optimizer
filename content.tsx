...
function findChatGPTToolbar(): HTMLElement | null {
  // Try to find the send button first
  const sendBtn = document.querySelector(
    'form button[data-testid*="send"], form button[aria-label*="Send"]'
  ) as HTMLElement | null
...
function findClaudeToolbar(): HTMLElement | null {
  ...
  const selectors = [
    ...
    'fieldset button:last-of-type',  // Often the send button is last
  ...
  if (!sendBtn) {
    // Try to find the input area instead and go from there
...
function ensureEmbeddedButtons() {
  ...
  // If all buttons already exist, skip
  if (document.getElementById(improveId) &&
  ...
  if (!next) {
    next = makeIconButton(nextId, "Next Prompt", "Next →")
